...
exports.addFavoriteArticle = tryCatch(async (req, res) => {
    ...
    if(!article) return res.status(404).json({result: false, error: "Article not found"});
    ...
    if (!user.favoritesArticles.includes(articleId)) {
        ...
        return res.status(409).json({result: true, message: "Article already in favorites"})
    }
...
exports.removeFavoriteArticle = tryCatch(async (req, res) => {
    ...
    user.favoritesArticles = user.favoritesArticles.filter( //je filter sur le tableau d'Id
    ...
    if (user.favoritesArticles.length === initialLength) {
        return res.status(404).json({result: false, message: "Article not found in favorites"})
    }
    res.status(200).json({result: true, message: "Article removed from favorites"});
});
